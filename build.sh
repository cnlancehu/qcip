...
do
    ...
    GOOS=$goos GOARCH=$goarch go build -o $output/qcip -ldflags "-X main.version=$version -X ain.goos=$goos -X main.goarch=$goarch -X main.buildtime=`date +%Y/%m/%d\ %H:%M:%S` -s -w"
